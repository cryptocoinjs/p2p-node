...
Message.prototype.checksum = function checksum() {
  return parseInt(sha256(Array.prototype.slice.apply(this.buffer.buffer.slice(0,this.cursor))).slice(0,8));
};
...
Peer.prototype.handleData = function handleData(data) {
  ...
  var checksumCalc = parseInt(sha256(Array.prototype.slice.apply(payload.buffer)).slice(0,8), 16);
  if (checksum != checksumCalc) {
    console.log('Supplied checksum of '+checksum+' does not match calculated checksum of '+checksumCalc);
  }
