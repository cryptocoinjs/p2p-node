...
Peer.prototype.handleData = function handleData(data) {
  ...
  var cmd = [];
  for (var i = 0; i < 12; i++) {
    var s = this.inbound.readUInt8(msgStart+4+i)
    if (s > 0) {
  ...
  var payloadLength = this.inbound.readUInt32LE(msgStart+16);
  var checksum = this.inbound.readUInt32BE(msgStart+20);
  
  if (this.inboundCursor < msgStart+payloadLength+24) return; // Not the whole message yet
  
  if (payloadLength > 0) {
    var payload = new Buffer(payloadLength);
    this.inbound.copy(payload, 0, msgStart+24, msgStart+24+payloadLength);
    var checksumCalc = new Buffer(sha256.x2(payload, {asBytes:true}));
    if (checksum != checksumCalc.readUInt32BE(0)) {
      console.log('Supplied checksum of '+checksum.toString(16)+' does not match calculated checksum of '+checksumCalc.toString(16));
    }
  } else {
